...
public class ProductVO 
{
	...
	private String p_iprice;	//	number
	private String p_oprice;	//	number
	private String p_image;		//	nvarchar2(125)
	
	/*
	 * DB연동을 할때 CHAR(1) TinyInt형으로 선언된 칼럼과 연동하기 위해 사용하는 
	 * type이 type 0 ~ 255, -128 ~ 127까지의 값만을 저장하는 데이터
	 * 메모리 공간 크기가 1byte(8bit)
	 * int형은 java에서 4byte 크기를 사용
	 * String형은 wrapper 클래스이다 보니 실제적으로 상당히 큰 메모리 공간을 차지한다.
	 * 단순히 flag와 같은 데이터를 취급할때는 char, byte형으로 사용하면 
	 * 메모리를 절약할수 있다.
	 */
	
	/*
	 * DB p_not_use 칼럼의 값이 null 이면 정상적인 데이터 이고
	 * null 이 아니면 (is not null)이면 삭제된 데이터로 취급하기로 하였다.
	 * update를 수행할때 VO의 칼럼에 값이 null이면 jdbcType.VARCHAR, config에서 수행한 설정때문에
