...
<script>
	$(function() 
	{
		/*
			...
			jquery에서 버튼이 클릭되었을때 실행할 event 핸들러를 만들고
...
<%@ include file ="/WEB-INF/views/include/include-header.jspf" %> <!-- Header -->
	...
	<section id="main">
		<form method = "POST">
			...
			<div><input name ="bl_user"     id ="bl_user"     value = "${BLOG.bl_user}"     placeholder="사용자를 입력하세요"/></div>
			<div><input name ="bl_title"    id ="bl_title"    value = "${BLOG.bl_title}"    placeholder="제목을 입력하세요"/></div>
			<div><input name ="bl_contents" id ="bl_contents" value = "${BLOG.bl_contents}" placeholder="내용을 입력하세요"/></div>
			<div>
